...
$MONGODB_CLIENT --port 27022 --tls --tlsCAFile=/etc/tls/certificates/ca.crt --tlsCertificateKeyFile=/etc/tls/certificates/client.pem --tlsAllowInvalidHostnames --eval "rs.initiate( { _id : 'rs0', members: [{ _id: 0, host: '${MONGO_REPLSET_HOST:-localhost}:27022' }, { _id: 1, host: '${MONGO_REPLSET_HOST:-localhost}:27023', priority: 0 }]})"
tail -f /dev/null
