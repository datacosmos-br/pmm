...
build-tarball: update-submodules
	...
	# Gera o tarball com prefixo "pmm-<shortcommit>/" (apenas arquivos rastreados)
	git archive --format=tar --prefix=$(PKG_NAME)-$(SHORT_COMMIT)/ HEAD > $(PKG_NAME)-$(SHORT_COMMIT).tar
